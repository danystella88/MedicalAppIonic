...
var gestisciErrori = function (Variabili, $state, $ionicPopup) {
  ...
  if (Variabili.user_info.id == undefined) {

    $ionicPopup.alert({
      title: 'Errore',
      template: "<b>Variabili.user_info.id</b> è <i>undefined</i>, probabilmente a causa di un refresh forzato dei dati"
    });
...
app.controller('LoginController', function ($scope, $state, $ionicLoading, $ionicPopup, Pazienti, Esami, Utenti, Variabili) {
  ...
  $scope.submit = function (username, password) {
   ...
   // window.plugins.OneSignal.getIds(function(ids) {
      ...
      var httpPost = Utenti.login(username, password);
  ...
  console.log("variabili user_info",Variabili.user_info)
...
app.controller('RegistrazioneController', function ($scope, $state, $ionicLoading, $ionicPopup, Utenti, Variabili) {
  ...
  $scope.submit = function (username,password,choice, nome, cognome, cfiscale, sesso, datadinascita, luogodinascita, provinciaNascita, patologia, specializzazione)
  {
    ...
    httpPost.success(function (response) {
      ...
      // entra direttamente con i tuoi dati
      var httpPost = Utenti.login(username, password);

      httpPost.success(function (response) {
        user_info = response;
        $scope.user_info = user_info;
        Variabili.user_info = user_info;
        ...
        var alertPopup = $ionicPopup.alert({
          title: 'Errore!',
          template: "C'è stato un problema con l'autologin. Prova manualmente."
        });
      }).finally(function($ionicLoading) {
        // On both cases hide the loading
        $scope.hide($ionicLoading);
...
app.controller('Medico_InfoAccountController', function ($scope, $state, $ionicPopup, Variabili) {
  ...
  $scope.logout = function () {
    ...
    Variabili = {};
    console.log("variabili del logout",Variabili.user_info)
    localStorage.setItem("loginType", '0');
...
app.controller('Paziente_InfoAccountController', function ($scope, $state, Variabili) {
  gestisciErrori(Variabili, $state);
